    ...
    def step_discrete(self, action_idx):
        ...
        for i, m in enumerate(milestones):
            if track_progress_ratio >= m and not self.milestone_rewards[i]:
                current_milestone_reward += 150.0  # High reward for 10% progress
        ...
        if not self.visited_far_zone and far_zone["xmin"] <= x <= far_zone["xmax"] and far_zone["ymin"] <= y <= far_zone["ymax"]:
            reward += 200.0  # <<< INCREASED FROM 500.0: Now a dominant early milestone
        ...
        if self.stagnation_steps > 5:
            reward -= 0.5 * self.stagnation_steps

        # --- Phase-based curriculum (REWARD CHANGES APPLIED) ---
        if gen < 10:
            # REMOVED: reward += (old_dist - new_dist) * 2.0 (Too focused on finish line)
            reward += 0.1 * movement
            # <<< NEW: Reward for furthest progress
            reward += 0.1 * self.max_distance_achieved
            # REMOVED: reward += 0.02 * self.distance_traveled

        elif gen < 20:
            # REMOVED: reward += (old_dist - new_dist) * 2.0
            reward += 0.1 * movement
            ...
            # Kept reward for finish line proximity for this phase
        ...
        # --- Remaining phases use a mix of local and checkpoint rewards (no continuous distance reward) ---
        elif gen < 40:
            reward += (old_dist - new_dist) * 2.0
            reward += 0.1 * movement
            # REMOVED: reward += 0.05 * self.distance_traveled (replaced by unified milestone)
            # REMOVED: old milestone block

        elif gen < 50:
            reward += (old_dist - new_dist) * 2.0
            reward += 0.1 * movement
            # REMOVED: reward += 0.05 * self.distance_traveled
